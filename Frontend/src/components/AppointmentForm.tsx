...
import { getDoctors, DoctorResponseDto } from "../apis/DoctorChatBotAPI";
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        console.log("Doctor list from API:", doctorList); // Debug
        if (!Array.isArray(doctorList)) {
          throw new Error("Dữ liệu bác sĩ không phải là mảng");
        }
        if (doctorList.length === 0) {
          toast.warning("Không có bác sĩ nào được tìm thấy");
        }
