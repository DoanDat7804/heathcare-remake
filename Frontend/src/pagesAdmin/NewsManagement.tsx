...
const NewsManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const data = await adminApi.getAllNews(); // Sử dụng adminApi
        setNews(data || []); // Đảm bảo luôn là mảng
      } catch (err: any) {
  ...
  const handleAddNews = async () => {
    try {
      await adminApi.createNews(newNews); // Gọi API tạo tin tức
      const data = await adminApi.getAllNews(); // Lấy lại danh sách
      setNews(data || []);
  ...
  const handleDeleteNews = async (id: string) => {
    if (window.confirm('Bạn có chắc muốn xóa tin tức này?')) {
      try {
        await adminApi.deleteNews(id); // Gọi API xóa tin tức
